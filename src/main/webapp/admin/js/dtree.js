...
dTree.prototype.add = function (id, pid, name, url, title, target, icon, iconOpen, open) {
    ...
    this.aNodes[this.aNodes.length] =
        new Node(id, pid, name, url, title, target, icon, iconOpen, open);
...
dTree.prototype.node = function (node, nodeId) {
    ...
    if (this.config.useIcons) {
        ...
        if (!node.icon) {
            node.icon =
                (this.root.id == node.pid) ? this.icon.root : ((node._hc) ? this.icon.folder
                    : this.icon.node);
        }
        ...
        str +=
            '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon)
            + '" alt="" />';
    ...
    if (node.url) {
        ...
        str +=
            '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is
        ...
        if (this.config.useStatusText) {
            str +=
                ' onmouseover="window.status=\'' + node.name
    ...
    if (node._hc) {
        ...
        str +=
            '<div id="d' + this.obj + nodeId + '" class="clip" style="display:' + ((this.root.id
                                                                                    == node.pid
                                                                                    || node._io)
                ? 'block' : 'none') + ';">';
...
dTree.prototype.indent = function (node, nodeId) {
    ...
    if (this.root.id != node.pid) {
        ...
        for (var n = 0; n < this.aIndent.length; n++) {
            str +=
                '<img src="' + ( (this.aIndent[n] == 1 && this.config.useLines) ? this.icon.line
                    : this.icon.empty ) + '" alt="" />';
        }
        ...
        if (node._hc) {
            ...
            str +=
                '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');"><img id="j' + this.obj
                + nodeId + '" src="';
            ...
            if (!this.config.useLines) {
                ...
                str +=
                    ( (node._io) ? ((node._ls && this.config.useLines) ? this.icon.minusBottom
            ...
            str +=
                '<img src="' + ( (this.config.useLines) ? ((node._ls) ? this.icon.joinBottom
...
dTree.prototype.nodeStatus = function (status, id, bottom) {
    ...
    eJoin.src = (this.config.useLines) ?
                ...
                ((status) ? ((bottom) ? this.icon.minusBottom : this.icon.minus) : ((bottom)
