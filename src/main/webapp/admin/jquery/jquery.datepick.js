...
(function ($) { // hide the namespace
    ...
    function Datepick() {
        ...
        this._defaults = { // Global defaults for all the date picker instances
            ...
            showCurrentAtPos: 0, // The position in multiple months at which to show the current
                                 // month (starting at 0)
    ...
    $.extend(Datepick.prototype, {
        ...
        _unselectableClass: 'datepick-unselectable', // The name of the unselectable cell marker
                                                     // class
        ...
        _destroyDatepick: function (target) {
            ...
            if (nodeName == 'input') {
                $target.siblings('.' + this._appendClass).remove().end()
                    ...
                    .removeClass(this.markerClassName).unbind('focus', this._showDatepick)
        ...
        _disableDatepick: function (target) {
            ...
            if (nodeName == 'input') {
                ...
                $target.siblings('button.' + this._triggerClass).each(function () {
                    this.disabled = true;
                }).end().siblings('img.' + this._triggerClass)
        ...
        _optionDatepick: function (target, name, value) {
            ...
            if (inst) {
                ...
                var date = new Date();
                extendRemove(inst, {
                    rangeStart: null, // start of range
        ...
        _doKeyDown: function (event) {
            ...
            if ($.datepick._datepickerShowing) {
                switch (event.keyCode) {
                    ...
                    case 13:
                        var sel = $('td.' + $.datepick._dayOverClass +
                                    ', td.' + $.datepick._currentClass, inst.dpDiv);
                        if (sel[0]) {
                            $.datepick._selectDay(event.target, inst.selectedYear,
                                                  inst.selectedMonth, sel[0]);
                        } else {
                    ...
                    case 33:
                        $.datepick._adjustDate(event.target, (event.ctrlKey ?
                                                              -$.datepick._get(inst,
                                                                               'stepBigMonths') :
                    ...
                    case 37:
                        ...
                        if (event.originalEvent.altKey) {
                            $.datepick._adjustDate(event.target,
                                                   (event.ctrlKey ? -$.datepick._get(inst,
                                                                                     'stepBigMonths')
                                                       :
        ...
        _showDatepick: function (input) {
            ...
            inst.dpDiv.css({
                               position: ($.datepick._inDialog && $.blockUI ?
            ...
            if (!inst.inline) {
                ...
                var postProcess = function () {
                    ...
                    if ($.browser.msie && parseInt($.browser.version, 10) < 7) { // fix IE < 7 select problems
                        ...
                        $('iframe.' + $.datepick._coverClass).css({
                                                                      width: inst.dpDiv.width()
                                                                             + extras[0],
                                                                      height: inst.dpDiv.height()
                                                                              + extras[1]
                                                                  });
                    }
                };
                if ($.effects && $.effects[showAnim]) {
                    inst.dpDiv.show(showAnim, $.datepick._get(inst, 'showOptions'), duration,
                                    postProcess);
                } else {
        ...
        _updateDatepick: function (inst) {
            ...
            var dims = {
                width: inst.dpDiv.width() + extras[0],
                height: inst.dpDiv.height() + extras[1]
            };
            inst.dpDiv.empty().append(this._generateHTML(inst)).find('iframe.' + this._coverClass)
        ...
        _checkOffset: function (inst, offset, isFixed) {
            ...
            if (this._get(inst, 'isRTL') || (offset.left + inst.dpDiv.width() - scrollX)
                                            > browserWidth) {
                offset.left = Math.max((isFixed ? 0 : scrollX),
                                       pos[0] + (inst.input ? inst.input.width() : 0) - (isFixed
                                           ? scrollX : 0) - inst.dpDiv.width() -
                                       (isFixed && $.browser.opera
        ...
        _checkExternalClick: function (event) {
            ...
            if (!$target.parents().andSelf().is('#' + $.datepick._mainDivId) && !$target.hasClass(
                    $.datepick.markerClassName) && !$target.parents().andSelf()
                    .hasClass($.datepick._triggerClass) &&
        ...
        _updateAlternate: function (inst) {
            ...
            if (altField) { // update alternate field too
                ...
                dateStr = (isArray(date) ? (!date[0] && !date[1] ? '' :
                                            this.formatDate(altFormat, date[0],
                                                            this._getFormatConfig(inst)) +
                                            this._get(inst, 'rangeSeparator') + this.formatDate(
                                                altFormat, date[1] || date[0],
                                                this._getFormatConfig(inst))) :
            ...
            if (inst.input) {
                inst.input.val(clear ? '' : this._formatDate(inst) +
                                            (!this._get(inst, 'rangeSelect') ? '' : this._get(inst,
                                                                                              'rangeSeparator')
                                            +
                                            this._formatDate(inst, inst.endDay, inst.endMonth,
                                                             inst.endYear)));
            }
        },
        ...
        _generateHTML: function (inst) {
            ...
            var clear = (this._get(inst, 'mandatory') ? '' :
                         '<div class="datepick-clear"><a onclick="jQuery.datepick._clearDate(\'#'
                         + inst.id + '\');"' +
            ...
            prevBigText = (!navigationAsDateFormat ? prevBigText : this.formatDate(prevBigText,
                                                                                   ...
                                                                                   this._getFormatConfig(
                                                                                       inst)));
            var prev = '<div class="datepick-prev">' + (this._canAdjustMonth(inst, -1, drawYear,
                                                                             drawMonth) ?
                       (showBigPrevNext ? '<a onclick="jQuery.datepick._adjustDate(\'#' + inst.id
                       + '\', -' + stepBigMonths + ', \'M\');"' +
                       this._addStatus(showStatus, inst.id, this._get(inst, 'prevBigStatus'),
                                       initStatus) + '>' + prevBigText + '</a>' : '') +
                       '<a onclick="jQuery.datepick._adjustDate(\'#' + inst.id + '\', -'
                       + stepMonths + ', \'M\');"' +
                       this._addStatus(showStatus, inst.id, this._get(inst, 'prevStatus'),
                                       initStatus) + '>' + prevText + '</a>' :
                                                        (hideIfNoPrevNext ? '&#xa0;'
                                                            : (showBigPrevNext ? '<label>'
                       + prevBigText + '</label>' : '') +
            ...
            nextBigText = (!navigationAsDateFormat ? nextBigText : this.formatDate(nextBigText,
                                                                                   ...
                                                                                   this._getFormatConfig(
                                                                                       inst)));
            var next = '<div class="datepick-next">' + (this._canAdjustMonth(inst, +1, drawYear,
                                                                             drawMonth) ?
                       '<a onclick="jQuery.datepick._adjustDate(\'#' + inst.id + '\', +'
                       + stepMonths + ', \'M\');"' +
                       ...
                       (showBigPrevNext ? '<a onclick="jQuery.datepick._adjustDate(\'#' + inst.id
                       + '\', +' + stepBigMonths + ', \'M\');"' +
                       this._addStatus(showStatus, inst.id, this._get(inst, 'nextBigStatus'),
                                       initStatus) + '>' + nextBigText + '</a>' : '') :
                                                        (hideIfNoPrevNext ? '&#xa0;' : '<label>'
                       + nextText + '</label>' +
                       (showBigPrevNext ? '<label>' + nextBigText + '</label>' : ''))) + '</div>';
            var currentText = this._get(inst, 'currentText');
            var gotoDate = (this._get(inst, 'gotoCurrent') && inst.currentDay ? currentDate
                : today);
            ...
            for (var row = 0; row < numMonths[0]; row++) {
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    html +=
                        '<div class="' + this._oneMonthClass + (col == 0 ? ' datepick-new-row' : '')
                        + '">' +
                        this._generateMonthYearHeader(inst, drawMonth, drawYear, minDate, maxDate,
                                                      selectedDate, row > 0 || col > 0, showStatus,
                                                      initStatus, monthNames) + // draw month
                                                                                // headers
                        '<table class="datepick" cellpadding="0" cellspacing="0"><thead>' +
                        '<tr class="datepick-title-row">' +
                        (showWeeks ? '<th' + this._addStatus(showStatus, inst.id, weekStatus,
                                                             initStatus) + '>' +
                        this._get(inst, 'weekHeader') + '</th>' : '');
                    for (var dow = 0; dow < 7; dow++) { // days of the week
                        var day = (dow + firstDay) % 7;
                        var dayStatus = (status.indexOf('DD') > -1 ? status.replace(/DD/,
                                                                                    dayNames[day]) :
                                         status.replace(/D/, dayNamesShort[day]));
                        html +=
                            '<th' + ((dow + firstDay + 6) % 7 >= 5
                                ? ' class="datepick-week-end-cell"' : '') + '>' +
                            (!changeFirstDay ? '<span' + this._addStatus(showStatus, inst.id,
                                                                         dayNames[day], initStatus)
                                :
                            '<a onclick="jQuery.datepick._changeFirstDay(\'#' + inst.id + '\', '
                            + day + ');"' +
                    ...
                    var leadDays = (this._getFirstDayOfMonth(drawYear, drawMonth) - firstDay + 7)
                                   % 7;
                    ...
                    for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
                        ...
                        for (var dow = 0; dow < 7; dow++) { // create date picker days
                            var daySettings = (beforeShowDay ?
                                               beforeShowDay.apply(
                                                   (inst.input ? inst.input[0] : null), [printDate])
                                : [true, '']);
                            ...
                            html += '<td class="datepick-days-cell' +
                                    ...
                                    + // highlight weekends
                                    (otherMonth ? ' datepick-other-month' : '') + // highlight days
                                                                                  // from other
                                                                                  // months
                                    ...
                                    (printDate.getTime() >= currentDate.getTime()
                                     && printDate.getTime() <= endDate.getTime() ? // in current
                                                                                   // range
                                    ...
                                    (!empty && daySettings[2] ? ' title="' + daySettings[2] + '"'
                                        : '') + // cell title
                                    ' onmouseover="' + (unselectable ? ''
                                    : 'jQuery(this).addClass(\'' + this._dayOverClass + '\');') + // current
                                    (highlightWeek ? 'jQuery(this).parent().addClass(\''
                                    + this._weekOverClass + '\');' : '') + // highlight week
                                    (!showStatus || empty ? '' : 'jQuery(\'#datepick-status-'
                                    + inst.id + '\').html(\'' + // show status
                                    (dateStatus.apply((inst.input ? inst.input[0] : null),
                                                      [printDate, inst]) || initStatus) + '\');') +
                                    (onHover && !empty ? 'jQuery.datepick._doHover(\'#' + // onHover
                                    inst.id + '\',' + printDate.getFullYear() + ','
                                    + printDate.getMonth() + ', this);' : '') + '"' +
                                    ' onmouseout="' + (unselectable ? ''
                                    : 'jQuery(this).removeClass(\'' + this._dayOverClass + '\');') + // not current
                                    (highlightWeek ? 'jQuery(this).parent().removeClass(\''
                                    + this._weekOverClass + '\');"' : '') + // unhighlight week
                                    (!showStatus || empty ? '' : 'jQuery(\'#datepick-status-'
                                    ...
                                    (empty ? '&#xa0;' : // not showing other months
                                     (unselectable ? printDate.getDate() : '<a>'
                                    + printDate.getDate() + '</a>')) + '</td>'; // display for this
                                                                                // month
            ...
            html +=
                (showStatus ? '<div style="clear: both;"></div><div id="datepick-status-' + inst.id
                +
        ...
        _generateMonthYearHeader: function (inst, drawMonth, drawYear, minDate, maxDate,
                                            selectedDate, secondary, showStatus, initStatus,
                                            monthNames) {
            minDate =
                (inst.rangeStart && minDate && selectedDate < minDate ? selectedDate : minDate);
            ...
            if (secondary || !changeMonth) {
                ...
                monthHtml += '<select class="datepick-new-month" ' +
                             'onchange="jQuery.datepick._selectMonthYear(\'#' + inst.id
                             + '\', this, \'M\');" ' +
        ...
        _isInRange: function (inst, date) {
            ...
            newMinDate =
                (newMinDate && inst.rangeStart < newMinDate ? inst.rangeStart : newMinDate);
    ...
    $.fn.datepick = function (options) {
        ...
        if (typeof options == 'string' && (options == 'isDisabled' || options == 'getDate')) {
            return $.datepick['_' + options + 'Datepick'].apply($.datepick,
                                                                [this[0]].concat(otherArgs));
        }
        return this.each(function () {
            typeof options == 'string' ?
            $.datepick['_' + options + 'Datepick'].apply($.datepick, [this].concat(otherArgs)) :
